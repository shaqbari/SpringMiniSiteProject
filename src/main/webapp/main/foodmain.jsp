...
$(document).ready(function() {
	...
	//Calculation for corners 4각형으로 나눠지는 부분
...
<style>
	.qitem {
		...
		background-size:cover;/*전체를 덮어준다. */
...
<div style="margin-left: 30px"><!-- 가운데 정렬하기 위해 민다. -->
<c:forEach var="hvo" items="${list }" varStatus="status">	
	<div class="qitem">
		<a href="fooddetail.do?no=${hvo.no }"><img src="${hvo.poster }" alt="${hvo.name }" title="" width="160" height="200"/></a>
		<span class="caption">
			<h4>${hvo.category } (${hvo.score })</h4>
			<p>${hvo.address}</p>
		</span>
	</div>
	
	<c:if test="${i%5==0}"> <!-- 다섯개마다 줄을 바꾼다. -->
		<div class="clear"></div>
	</c:if>
